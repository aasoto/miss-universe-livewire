    ...
    <div class="pt-24 flex flex-col justify-center items-center">
        ...
        <div class="grid grid-cols-6 lg:grid-cols-12 gap-3 mb-3 w-full">
            <div class="col-span-5">
                <a href="{{ route('d-editions-miss-photogenic-create') }}">
                    <button
        ...
        <table class="container mx-0 w-[85%] sm:w-full">
            ...
            <tbody>
                @foreach ($photogenics as $photogenic)
                <tr class="w-full border-b text-xs iPhoneSE:text-base border-gray-300">
                    ...
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base font-bold break-words iPhoneSE:break-normal">
                        {{ $photogenic->edition->year }}
                    </td>
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        {{ $photogenic->edition->name }}
                    </td>
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base font-bold break-words iPhoneSE:break-normal">
                        {{ __($photogenic->candidate->country->name) }}
                    </td>
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        {{ $photogenic->candidate->first_name.' '.$photogenic->candidate->second_name.' '.$photogenic->candidate->first_last_name.' '.$photogenic->candidate->second_last_name }}
                    </td>
                    <td class="flex gap-1 justify-center items-center px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        ...
                        <a href="{{ route('d-editions-miss-photogenic-edit', $photogenic) }}">
                            <button
                        ...
                        <button wire:click="selectedPhotogenicToDelete({{ $photogenic }})"
        ...
        <x-jet-confirmation-modal wire:model="confirmingDeletePhotogenic">
            <x-slot name="title">
                <div class="">
                    {{ __('Delete Miss Photogenic') }}
                </div>
            </x-slot>
