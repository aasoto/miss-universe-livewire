<section class="pt-32 pb-10">
        ...
        <tbody>
            @foreach ($editions as $edition)
                <tr class="w-full border-b text-xs iPhoneSE:text-base border-gray-300">
                    ...
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        {{ __($edition->official_name) }}
                    </td>
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        {{ __($edition->broadcaster->name) }}
                    </td>
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        <div class="text-center">
                            <button wire:click="confirmAction({{$edition->id}})" class="w-full rounded-full px-4 py-2 bg-gradient-to-r from-lime-400 dark:from-lime-200 to-green-900 dark:to-green-700 text-white text-medium font-bold hover:bg-green-700 hover:scale-110 transition duration-200">
                                {{__('Edition')}}
                            </button>
                        </div>
                    </td>
    ...
    <x-modals.question-modal wire:model="confirming_edition">
        <x-slot name="title">
            <div class="">
                {{ __('Confirm attach') }}
            </div>
        ...
        <x-slot name="content">
            {{ __('Are you sure that this is the presenter of this edition?') }}
        </x-slot>
        <x-slot name="footer">
            <button class=""></button>
    ...
    <x-modals.success-modal wire:model="success">
        ...
        <x-slot name="content">
            {{ __('The attach has been saved successfully.') }}
        </x-slot>
        <x-slot name="footer">
            <button class=""></button>
