@slot('header')
{{__('Editions - Entertainment show')}}
@endslot

<x-card.card-primary class="items-center">
    @slot('title')
        {{__('Entertainment show')}}
    @endslot
    <x-jet-action-message on="deleted">
        <div class="box-success-action-message">
            {{__('Entertainment show deleted successfully')}}
        </div>
        ...
        <div class="col-span-2">
            <a href="{{route('d-editions-entertainment-show-create')}}">
                <x-btn.button-success class="my-2">
                    {{__('Nuevo show de entretenimiento')}}
    ...
    <table class="table w-full">
        ...
        <tbody>
            @foreach ($entraiment_shows as $entraiment)
                <tr class="border-b">
                    <td class="p-2">{{ $entraiment->id }}</td>
                    <td class="p-2">{{ $entraiment->artist }}</td>
                    <td class="p-2">{{ $entraiment->country->name }}</td>
                    <td class="p-2">{{ $entraiment->edition->name }}</td>
                    <td class="p-2">
                    ...
                    <td class="p-2">
                        <a href="{{ route('d-editions-entertainment-show-edit', $entraiment) }}" class="mr-2">Editar</a>
                        <x-jet-danger-button
                            wire:click="selectedEntraimentShowToDelete({{ $entraiment }})">
                            Eliminar
                        </x-jet-danger-button>
                    </td>
                </tr>
