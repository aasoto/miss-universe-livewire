            ...
            @foreach ($quarterfinalists as $quarterfinalist)
                <tr class="w-full border-b text-xs iPhoneSE:text-base border-gray-300">
                    ...
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base font-bold break-words iPhoneSE:break-normal">
                        {{ $quarterfinalist->edition->year }}
                    </td>
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        {{ $quarterfinalist->edition->name }}
                    </td>
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base font-bold break-words iPhoneSE:break-normal">
                        {{ __($quarterfinalist->candidate->country->name) }}
                    </td>
                    <td class="px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        {{ $quarterfinalist->candidate->first_name.' '.$quarterfinalist->candidate->second_name.' '.$quarterfinalist->candidate->first_last_name.' '.$quarterfinalist->candidate->second_last_name }}
                    </td>
                    <td class="flex gap-1 justify-center items-center px-1 sm:px-2 py-3 text-xs md:text-base break-words iPhoneSE:break-normal">
                        <button wire:click="selectedQuarterfinalistToDelete({{ $quarterfinalist }})"
