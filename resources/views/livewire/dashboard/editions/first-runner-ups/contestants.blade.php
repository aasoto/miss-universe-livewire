            ...
            <tr class="border-b">
                ...
                <td class="p-2">
                    <div class="text-center">
                        <button wire:click="confirmAction({{$contestant->candidate->id}})" class="m-1 p-2 bg-green-500 hover:bg-green-800 hover:cursor-pointer rounded-md shadow-md shadow-gray-300 font-bold text-white">{{__('First runner up')}}</button>
                    </div>
                </td>
            </tr>
        @endforeach
...
<x-modals.question-modal wire:model="confirming_first_runner_up">
    <x-slot name="title">
        <div class="">
            {{ __('Confirm first runner up') }}
        </div>
    </x-slot>
    <x-slot name="content">
        {{ __('Are you sure that this contestant is the first runner up of this edition?') }}
    </x-slot>
    <x-slot name="footer">
        <button class=""></button>
