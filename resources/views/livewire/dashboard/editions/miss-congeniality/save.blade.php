...
<div class = "items-center flex flex-col sm:justify-center pt-6 sm:pt-0 bg-gray-100">
    <div class="w-full sm:w-11/12 mt-6 bg-white shadow-md overflow-hidden sm:rounded-lg">
        <div class="bg-blue-500 py-3">
            <h2 class="text-3xl text-center text-white">{{__('Miss Congeniality')}}</h2>
        </div>
        <div class="px-6 py-4">
            ...
            @if ($edition_id)
            @livewire('dashboard.editions.miss-congeniality.contestants', ['edition_id' => $edition_id, 'edit_mode' => $edit_mode])
            @endif
            <x-modals.success-modal wire:model="success">
                ...
                <x-slot name="content">
                    {{ __('The miss congeniality has been saved successfully.') }}
                </x-slot>
                <x-slot name="footer">
                    <button class=""></button>
            ...
            <x-modals.error-modal wire:model="exist">
                <x-slot name="title">
                    <div class="">
                        {{ __('The Miss Congeniality of this edition has been choosen!') }}
                    </div>
                </x-slot>
                <x-slot name="content">
