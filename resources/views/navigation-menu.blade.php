<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden sm:items-center sm:ml-10 sm:flex">
                    <x-jet-nav-link href="{{ route('dashboard') }}" :active="request()->routeIs('dashboard')" class="mr-2">
                        {{ __('Dashboard') }}
                    </x-jet-nav-link>
                    <x-jet-nav-link href="{{ route('d-candidate-index') }}" :active="request()->routeIs('d-candidate-index')" class="mr-2">
                        {{ __('Candidate') }}
                    </x-jet-nav-link>
                        ...
                        <x-slot name="content">
                            <div class="w-60">
                                ...
                                <div class="block px-4 py-2 text-xs text-gray-400">
                                    {{ __('Manage National Committees') }}
                                </div>

                                <!-- Team Settings -->
                                <x-jet-dropdown-link href="{{ route('d-nationalcommittee-create') }}">
                                    {{ __('Create') }}
                                </x-jet-dropdown-link>
                    ...
                    <x-jet-nav-link href="{{ route('d-carousel-index') }}" :active="request()->routeIs('d-carousel-index')" class="mr-2">
                        {{ __('Carousel') }}
                    </x-jet-nav-link>
                    <x-jet-dropdown align="left" width="60">
                        <x-slot name="trigger">
                        ...
                        <x-slot name="content">
                            <div class="w-60">
                                ...
                                <!-- Team Settings -->
                                <x-jet-dropdown-link href="{{ route('d-editions-broadcaster-index') }}">
                                    {{ __('Broadcasters') }}
                                </x-jet-dropdown-link>
                                <x-jet-dropdown-link href="{{ route('d-editions-city-venue-index') }}">
                                    {{ __('City venues') }}
                                </x-jet-dropdown-link>
                                <x-jet-dropdown-link href="{{ route('d-editions-place-index') }}">
                                    {{ __('Places') }}
                                </x-jet-dropdown-link>
                                <x-jet-dropdown-link href="{{ route('d-editions-presenter-index') }}">
                                    {{ __('Presenters') }}
                                </x-jet-dropdown-link>
                        ...
                        <x-slot name="trigger">
                            @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                <button class="flex text-sm border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300 transition">
                                    <img class="h-8 w-8 rounded-full object-cover" src="{{url('/')}}/../storage/app/public/{{ Auth::user()->profile_photo_path }}" alt="{{ Auth::user()->name }}" />
                                </button>
                            @else
                                <span class="inline-flex rounded-md">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-2 pb-3 space-y-1">
            <x-jet-responsive-nav-link href="{{ route('d-candidate-index') }}" :active="request()->routeIs('d-candidate-index')">
                {{ __('Candidates') }}
            </x-jet-responsive-nav-link>
        </div>
        ...
        <!-- National committees options -->
        <x-jet-responsive-nav-link href="{{ route('d-nationalcommittee-index', Auth::user()->currentTeam->id) }}" :active="request()->routeIs('teams.show')">
            {{ __('Index') }}
        </x-jet-responsive-nav-link>
        <x-jet-responsive-nav-link href="{{ route('d-nationalcommittee-create', Auth::user()->currentTeam->id) }}" :active="request()->routeIs('teams.show')">
            {{ __('Create') }}
        </x-jet-responsive-nav-link>

        <div class="border-t border-gray-200"></div>
        <div class="pt-2 pb-3 space-y-1">
            <x-jet-responsive-nav-link href="{{ route('d-news-index') }}" :active="request()->routeIs('d-news-index')">
                {{ __('News') }}
            </x-jet-responsive-nav-link>
        </div>
            ...
            <div class="flex items-center px-4">
                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                    <div class="shrink-0 mr-3">
                        <img class="h-10 w-10 rounded-full object-cover" src="{{url('/')}}/../storage/app/public/{{ Auth::user()->profile_photo_path }}" alt="{{ Auth::user()->name }}" />
                    </div>
                @endif
